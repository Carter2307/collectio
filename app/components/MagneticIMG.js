...
import { lerp } from "../utils/function";
...
export default class MagneticIMG extends Components {
  ...
  onMouseOver(e) {
    ...
    this.changeImage(e.currentTarget, e.currentTarget.getAttribute('data-snap-image'))
  }
  ...
  update() {
    this.position.currentx = lerp(this.position.currentx , this.position.targetx - this.element.offsetWidth/2, .1)
    this.position.currenty = lerp(this.position.currenty, this.position.targety - this.element.offsetHeight/2, .1)
